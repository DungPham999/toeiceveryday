(() => {
  ...
  let checkedGroups = new Set(
    loadJSON('ets2024-t1-checked-groups', [])
  );
  let marked = new Set(
    loadJSON('ets2024-t1-marked', [])
  );
  let scriptOverrides = loadJSON(
    'ets2024-t1-script-overrides',
    {}
  );

  const byPart = p => data.filter(x => x.part === p);

  const allQuestions = data.flatMap(x =>
    x.questions.map(q => ({
      q,
      item: x
    }))
  ...
  function saveState() {
    ...
    localStorage.setItem(
      'ets2024-t1-checked-groups',
      JSON.stringify([...checkedGroups])
    );
    ...
    localStorage.setItem(
      'ets2024-t1-script-overrides',
      JSON.stringify(scriptOverrides)
    );
  ...
  function groupDone(item = currentItem) {
  ...
  function groupChecked(item = currentItem) {
    ...
    return item.part <= 2
      ? groupDone(item)
      : checkedGroups.has(item.id);
  }

  function questionChecked(q, item) {
    return item.part <= 2
      ? isAnswered(q)
      : checkedGroups.has(item.id);
  }

  function questionCorrect(q, item) {
    return (
      questionChecked(q, item) &&
      answers[q.n] === q.answer
    );
  }

  function questionWrong(q, item) {
    return (
      questionChecked(q, item) &&
      answers[q.n] !== q.answer
    );
  ...
  function setPart(p) {
    ...
    focusQuestionNumber =
      byPart(part)[0]?.questions[0]?.n ?? null;
  ...
  function render() {
    ...
    if (
      !currentItem.questions.some(
        q => q.n === focusQuestionNumber
      )
    ) {
      focusQuestionNumber =
        currentItem.questions[0].n;
    }
  ...
  function renderVisual() {
    ...
    if (src) {
      ...
      img.alt = currentItem.image
        ? 'TOEIC Part 1 picture'
        : 'Question graphic';
  ...
  function renderQuestions() {
    ...
    if (part <= 2 && !checked) {
      ...
      hint.textContent =
        'Nghe trước. Câu hỏi/đáp án tiếng Anh sẽ hiện sau khi bạn chọn.';
    ...
    if (part >= 3 && !checked) {
      ...
      btn.textContent =
        groupDone()
          ? 'Check'
          : `Chọn đủ ${currentItem.questions.length} câu để Check`;
  ...
  function renderScript() {
    ...
    editScriptBtn.textContent =
      editingScript
        ? '✓ Lưu sửa'
        : '✎ Sửa script';

    scriptEditNote.classList.toggle(
      'hidden',
      !editingScript
    );

    exportCorrectionsBtn.disabled =
      Object.keys(
        scriptOverrides
      ).length === 0;
  }
  ...
  exportCorrectionsBtn.onclick = () => {
    const payload = {
      note:
        'Script corrections exported from TOEIC Prototype',
      exportedAt:
