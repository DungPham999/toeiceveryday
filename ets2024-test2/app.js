(() => {
  ...
  const storageBase = config.storageKey || 'toeic-listening-test';
  const STORAGE = {
    answers: `${storageBase}-answers`,
    checkedGroups: `${storageBase}-checked-groups`,
    marked: `${storageBase}-marked`,
    scriptOverrides: `${storageBase}-script-overrides`
  };
  const testTitle = config.title || `ETS ${config.year || ''} · Test ${config.testNumber || ''}`.replace(/\s+/g,' ').trim();
  const data = window.TEST_DATA;
  if (!Array.isArray(data)) {
    document.body.innerHTML = '<main style=\"padding:40px;font-family:system-ui\"><h2>Không tải được dữ liệu đề.</h2><p>Kiểm tra file data.js.</p></main>';
    throw new Error('TEST_DATA is missing or invalid');
  }
  ...
  const testTitleText = document.getElementById('testTitleText');
  const paletteKicker = document.getElementById('paletteKicker');
  const homeLink = document.getElementById('homeLink');
  document.title = `TOEIC Listening Full Test – ${testTitle}`;
  if (testTitleText) testTitleText.textContent = testTitle;
  if (paletteKicker) paletteKicker.textContent = testTitle.toUpperCase();
  if (homeLink && config.homeUrl) homeLink.href = config.homeUrl;
  ...
  let checkedGroups = new Set(loadJSON(STORAGE.checkedGroups, []));
  let marked = new Set(loadJSON(STORAGE.marked, []));
  let scriptOverrides = loadJSON(STORAGE.scriptOverrides, {});
  ...
  function saveState(){
    ...
    localStorage.setItem(STORAGE.checkedGroups, JSON.stringify([...checkedGroups]));
    localStorage.setItem(STORAGE.marked, JSON.stringify([...marked]));
    localStorage.setItem(STORAGE.scriptOverrides, JSON.stringify(scriptOverrides));
    updateProgress();
    renderPalette();
  }
  ...
  exportCorrectionsBtn.onclick = () => {
    const payload = {
      note: `Script corrections exported from ${testTitle}`,
    ...
    a.href = url;
    a.download = `script-corrections-${storageBase}.json`;
  ...
  // Làm lại toàn bộ bài Listening.
  // Giữ nguyên các script correction đã sửa, chỉ xóa tiến độ làm bài.
  if (resetBtn) {
    resetBtn.onclick = () => {
      const ok = window.confirm(
        'Bạn có chắc muốn làm lại toàn bộ bài Listening từ đầu?\n\n' +
        'Toàn bộ đáp án, kết quả đúng/sai và câu đã đánh dấu sẽ được xóa.'
      );
      if (!ok) return;
      ...
      localStorage.removeItem(STORAGE.checkedGroups);
      localStorage.removeItem(STORAGE.marked);

      part = 1;
      indexInPart = 0;
      focusQuestionNumber = byPart(1)[0]?.questions[0]?.n ?? null;
